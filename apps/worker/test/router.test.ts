...
const ENV: Env = {
  ...
  ROUTER_TOOL_MODEL: 'gemini-3-flash-preview',
...
describe('resolveGeminiModel', () => {
  it('passes gemini- ids through unchanged', () => {
    expect(resolveGeminiModel('gemini-3-flash-preview', ENV)).toBe('gemini-3-flash-preview');
  ...
  it('maps legacy Anthropic hints to Gemini tiers', () => {
    ...
    expect(resolveGeminiModel('claude-sonnet-4-6', ENV)).toBe('gemini-3-flash-preview');
    expect(resolveGeminiModel('claude-opus-4-7', ENV)).toBe('gemini-3-flash-preview');
  });

  it('falls back to default for unknown labels', () => {
    expect(resolveGeminiModel('gpt-4o', ENV)).toBe('gemini-3-flash-preview');
    expect(resolveGeminiModel('', ENV)).toBe('gemini-3-flash-preview');
  });
});
